...
def get_extra_score_from_hand(hand):
  ...
  return int(new_hand.replace("T", "10").replace("J", "01").replace("Q", "11").replace("K", "12").replace("A", "13"))

def get_score_from_hand(hand):
  max = 0
  for card_type in card_types:
    new_score = get_base_score_from_hand(hand.replace("J", card_type))
    if new_score > max:
      max = new_score
  return max + get_extra_score_from_hand(hand)
